...
## Canonical Rules
Always read and apply, in order:
...
Task-specific:
- API contract tasks must also follow `.agents/api-contract-guide.md`.
...
5. Run relevant validation (`make test`, `make lint`, `make fmt` when needed).
...
2. Update entry/bridge files only to reference `.agents/`.
