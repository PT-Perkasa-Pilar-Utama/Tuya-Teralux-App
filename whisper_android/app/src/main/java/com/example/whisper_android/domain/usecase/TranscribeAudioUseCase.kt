...
class TranscribeAudioUseCase(
    ...
    suspend operator fun invoke(audioFile: File, token: String, language: String): Flow<Resource<String>> = flow {
        ...
        speechRepository.pollTranscription(taskId!!, token).collect { result ->
            when (result) {
                ...
                is Resource.Error -> emit(Resource.Error(result.message ?: "Transcription polling failed"))
                is Resource.Loading -> emit(Resource.Loading())
            }
        }
    }
