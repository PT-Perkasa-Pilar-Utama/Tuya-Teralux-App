...
def create_temp_password(access_token, ticket_id, encrypted_password, password, valid_hours=1):
...
def generate_password(password="7654321", valid_hours=1):
    ...
    result, body = create_temp_password(access_token, ticket_id, encrypted_password, password, valid_hours)
    ...
    if result.get('success'):
        ...
        print(f"   Valid: {valid_hours} hour(s) from now")
...
if __name__ == "__main__":
    ...
    valid_hours = int(sys.argv[2]) if len(sys.argv) > 2 else 1
    
    success, password_id = generate_password(password, valid_hours)
