...
type NotificationSchedulerWorker struct {
	scheduledRepo   *notificationRepositories.ScheduledNotificationRepository
	waSvc           *services.WANotificationService
	templateService *services.WATemplateService
	stopCh          chan struct{}
	wg              sync.WaitGroup
	running         uint32
}
...
func NewNotificationSchedulerWorker(waBaseURL string) *NotificationSchedulerWorker {
	return &NotificationSchedulerWorker{
		...
		waSvc:           services.NewWANotificationService(waBaseURL),
		templateService: services.NewWATemplateService(),
...
func (w *NotificationSchedulerWorker) processNotification(notification *notificationEntities.ScheduledNotification) {
...
func (w *NotificationSchedulerWorker) buildWAMessage(bookingInfo map[string]interface{}, notification *notificationEntities.ScheduledNotification) string {
	...
	templateData := &services.WATemplateData{
...
func (w *NotificationSchedulerWorker) buildFallbackMessage(data *services.WATemplateData) string {
...
func (w *NotificationSchedulerWorker) calculateRemainingMinutes(notification *notificationEntities.ScheduledNotification) string {
	if notification.BookingTimeEnd == "" {
		return "15"
	}
