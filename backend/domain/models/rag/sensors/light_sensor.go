...
func (s *LightSensor) ExecuteControl(token string, device *tuyaDtos.TuyaDeviceDTO, prompt string, history []string, executor interfaces.DeviceControlExecutor) (*dtos.ControlResultDTO, error) {
	...
	if code, value, matched := s.matchLightControl(promptLower, device.Status); matched {
		...
		return &dtos.ControlResultDTO{
			Message:  fmt.Sprintf("Berhasil mengatur %s di %s.", code, device.Name),
			DeviceID: device.ID,
		}, nil
	}
