...
func TestTranslateUseCase_Execute(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		u := NewTranslateUseCase(mockLLM, nil, cfg, nil, store, nil, &SimpleMockSkill{SkillName: "Translation"})
	...
	t.Run("Empty Config Model fallback", func(t *testing.T) {
		...
		mockLLM := &mockLLMForTranslate{}
		u := NewTranslateUseCase(mockLLM, nil, emptyCfg, nil, store, nil, &SimpleMockSkill{SkillName: "Translation"})
