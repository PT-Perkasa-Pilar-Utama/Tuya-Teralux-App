...
type SpeechModelsOrionController struct {
	usecase       usecases.TranscribeOrionModelUseCase
	saveRecording recordingUsecases.SaveRecordingUseCase
	config        *utils.Config
}

func NewSpeechModelsOrionController(
	usecase usecases.TranscribeOrionModelUseCase,
	saveRecording recordingUsecases.SaveRecordingUseCase,
	cfg *utils.Config,
) *SpeechModelsOrionController {
	return &SpeechModelsOrionController{
		usecase:       usecase,
...
// Transcribe handles POST /api/speech/models/orion
...
// @Router /api/speech/models/orion [post]
func (c *SpeechModelsOrionController) Transcribe(ctx *gin.Context) {
	...
	if err != nil {
		utils.LogError("Orion.SaveRecording: %v", err)
