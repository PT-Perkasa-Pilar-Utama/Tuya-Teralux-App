...
type SpeechModelsOpenAIController struct {
	usecase       usecases.TranscribeOpenAIModelUseCase
	saveRecording recordingUsecases.SaveRecordingUseCase
	config        *utils.Config
}

func NewSpeechModelsOpenAIController(
	usecase usecases.TranscribeOpenAIModelUseCase,
	saveRecording recordingUsecases.SaveRecordingUseCase,
	cfg *utils.Config,
) *SpeechModelsOpenAIController {
	return &SpeechModelsOpenAIController{
		usecase:       usecase,
...
// Transcribe handles POST /api/speech/models/openai
...
// @Router /api/speech/models/openai [post]
func (c *SpeechModelsOpenAIController) Transcribe(ctx *gin.Context) {
