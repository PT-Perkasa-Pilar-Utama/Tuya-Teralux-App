...
type IDeviceRepository interface {
	...
	GetByTerminalID(terminalID string) ([]entities.Device, error)
	GetByTerminalIDPaginated(terminalID string, offset, limit int) ([]entities.Device, int64, error)
...
// GetByTerminalID retrieves all devices belonging to a specific Terminal unit
func (r *DeviceRepository) GetByTerminalID(terminalID string) ([]entities.Device, error) {
	if r.db == nil {
		return nil, fmt.Errorf("database not initialized")
	}
	var devices []entities.Device
	err := r.db.Where("terminal_id = ?", terminalID).Find(&devices).Error
	return devices, err
}

// GetByTerminalIDPaginated retrieves devices by Terminal ID with pagination
func (r *DeviceRepository) GetByTerminalIDPaginated(terminalID string, offset, limit int) ([]entities.Device, int64, error) {
	if r.db == nil {
		return nil, 0, fmt.Errorf("database not initialized")
	}
	var devices []entities.Device
	...
	// Count total
	if err := r.db.Model(&entities.Device{}).Where("terminal_id = ?", terminalID).Count(&total).Error; err != nil {
		return nil, 0, err
	}

	// Fetch paginated
	query := r.db.Where("terminal_id = ?", terminalID)
	if limit > 0 {
		query = query.Offset(offset).Limit(limit)
	}
	err := query.Find(&devices).Error
