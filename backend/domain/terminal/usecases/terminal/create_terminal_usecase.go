...
func (uc *CreateTerminalUseCase) CreateTerminal(req *dtos.CreateTerminalRequestDTO) (*dtos.CreateTerminalResponseDTO, bool, error) {
	// Normalization
	...
	if err == nil && existing != nil {
		utils.LogDebug("CreateTerminalUseCase: MAC conflict detected for %s", req.MacAddress)
		return nil, false, utils.NewAPIError(409, "Mac Address already in use")
	}

	// Call external service before saving to DB
	utils.LogDebug("CreateTerminalUseCase: Attempting external registration for MAC: %s", req.MacAddress)
	if err := uc.externalService.ProcInsertMacAddress(roomIDInt, req.MacAddress, deviceTypeIDInt); err != nil {
		utils.LogError("CreateTerminalUseCase: External registration failed for MAC %s: %v", req.MacAddress, err)
		return nil, false, fmt.Errorf("failed to register device externally: %w", err)
	}
	utils.LogDebug("CreateTerminalUseCase: External registration success for MAC: %s", req.MacAddress)
	...
	// Save to database
	if err := uc.repository.Create(terminal); err != nil {
		utils.LogError("CreateTerminalUseCase: DB save failed for MAC %s: %v", req.MacAddress, err)
