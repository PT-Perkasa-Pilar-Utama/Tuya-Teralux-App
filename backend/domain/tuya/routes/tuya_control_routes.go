...
func SetupTuyaControlRoutes(router gin.IRouter, switchController *controllers.TuyaCommandSwitchController, sendIRController *controllers.TuyaSendIRCommandController) {
	...
	api := router.Group("/api/tuya")
	{
		...
		api.POST("/devices/:id/commands/switch", switchController.SendSwitchCommand)

		// POST /api/tuya/devices/:id/commands/ir
		// Sends an infrared command (e.g., AC control) to an IR-enabled device.
		api.POST("/devices/:id/commands/ir", sendIRController.SendIRACCommand)
	}
}
