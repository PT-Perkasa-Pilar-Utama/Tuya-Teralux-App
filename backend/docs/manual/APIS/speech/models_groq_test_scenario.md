# ENDPOINT: POST /api/speech/models/groq

## Description
Submits an audio file for transcription via the **Groq Whisper** model. This endpoint uses the Groq API and is processed **asynchronously**.
...
```
- **Pre-conditions**: Audio file is valid and Groq API is accessible.
...
- **Side Effects**: 
  ...
  - Background processing started via Groq.
...
- **Expected Behavior**: File accepted at API layer, but Groq API will return an error during background processing.
