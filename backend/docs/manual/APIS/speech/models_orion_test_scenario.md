# ENDPOINT: POST /api/speech/models/orion

## Description
Submits an audio file for transcription via the **Orion (Outsystems)** model. This endpoint explicitly uses the Orion service and is processed **asynchronously**.
...
### 1. Transcribe via Orion (Success)
- **Method**: `POST`
- **Headers**:
```json
{
...
```
- **Pre-conditions**: Audio file is valid and Orion service is reachable.
- **Request**: Upload `audio.m4a`.
- **Expected Response**:
```json
{
  "status": true,
  "message": "Orion transcription task submitted",
  "data": {
    "task_id": "orion-abc123-xyz789",
    "task_status": "pending",
...
- **Side Effects**: 
  ...
  - Background processing started via Orion.
...
- **Expected Behavior**: File accepted at API layer, but Orion service will return an error during background processing.
